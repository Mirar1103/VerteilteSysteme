...
public class Gui extends JPanel implements ActionListener {
	 ...
	 *
	 * @param phil
	 *            The index of the philosopher (between 0 and n-1)
	 * @param state
	 *            One of: {@link DiningPhilosophersDisplay#THINKING},
	 *            {@link DiningPhilosophersDisplay#HUNGRY},
	 *            {@link DiningPhilosophersDisplay#EATING}
	 * @throws IllegalArgumentException
	 *             If an invalid fork or philosopher index or an invalid state
	 *             is given
	 */
	public void setPhilosopherState(int phil, int state)
			throws IllegalArgumentException {
		if ((state != Gui.THINKING)
				&& (state != Gui.HUNGRY)
				&& (state != Gui.EATING)) {
	 ...
	 *
	 * @param fork
	 *            The index of the fork (between 0 and n-1)
	 * @param phil
	 *            The index of the philosopher: either equal to fork, or to
	 *            (fork + 1) modulo n
	 * @throws IllegalArgumentException
	 *             If an invalid fork or philosopher index is given
	 */
	// Give a fork to a philosopher
	public void setForkTaken(int fork, int phil)
			throws IllegalArgumentException {
		...
		if ((phil != fork) && (phil != (fork + 1) % num_phils)) {
			throw new IllegalArgumentException("Philosopher " + phil
					+ " cannot take fork " + fork + "!");
		}
	 ...
	 *            The index of the fork (between 0 and n-1)
	 * @throws IllegalArgumentException
	 *             If an invalid fork index is given
	 */
	// Place a fork back on the table
	public void setForkAvailable(int fork) throws IllegalArgumentException {
		...
		System.out.println("Philosopher " + forks[fork] + " relases fork "
				+ fork);
		forks[fork] = -1;
		this.repaint();
	}
	...
	/**
	 * The actionPerformed method, which should not be overriden or called
	 * directly.
	 */
	...
	private class TablePanel extends JPanel {
		/**
		 * The paintComponent method, which should not be overriden or called
		 * directly.
		 */
		public void paintComponent(Graphics win) {

			int center_x = (Gui.size / 2);
			int center_y = (Gui.size / 2);
			int phil_location_radius = (int) (((Gui.size / 2) - Gui.default_phil_radius) * .90);
			...
			int phil_radius = java.lang.Math
					.min(default_plate_radius, (int) (2 * java.lang.Math.PI
							* phil_location_radius / (2 * num_phils)));
			int plate_radius = java.lang.Math
					.min(default_plate_radius, (int) (2 * java.lang.Math.PI
							* plate_location_radius / (2 * num_phils)));
			...
			for (int i = 0; i < num_phils; i++) {
				...
				draw_circle(
						win,
						Color.white,
						(int) (center_x + plate_location_radius
								* java.lang.Math.cos(i * 2 * java.lang.Math.PI
										/ num_phils)),
						(int) (center_y + plate_location_radius
								* java.lang.Math.sin(i * 2 * java.lang.Math.PI
										/ num_phils)),
						java.lang.Math
								.min(plate_radius,
										(int) (2 * java.lang.Math.PI
												* plate_location_radius / (2 * num_phils))));
				// pick the right color for the philosopher
				Color phil_color = Color.white; // white is the color of a
												// philosopher in an invalid
												// state
				...
				draw_circle(
						win,
						phil_color,
						(int) (center_x + phil_location_radius
								* java.lang.Math.cos(i * 2 * java.lang.Math.PI
										/ num_phils)),
						(int) (center_y + phil_location_radius
								* java.lang.Math.sin(i * 2 * java.lang.Math.PI
										/ num_phils)),
						java.lang.Math
								.min(phil_radius,
				...
				if (num_phils < 50) {
					win.drawString(
							...
							(int) (center_x + phil_location_radius
									* java.lang.Math.cos(i * 2
											* java.lang.Math.PI / num_phils)),
							(int) (center_y + phil_location_radius
									* java.lang.Math.sin(i * 2
											* java.lang.Math.PI / num_phils)));
				}
			}
			// Draw the chopstick
			...
			for (int i = 0; i < num_phils; i++) {
				if (forks[i] == -1) {
					win.drawLine(
							(int) (center_x + (plate_location_radius + plate_radius)
									* java.lang.Math.cos((i + .5) * 2
											* java.lang.Math.PI / num_phils)),
							(int) (center_y + (plate_location_radius + plate_radius)
									* java.lang.Math.sin((i + .5) * 2
											* java.lang.Math.PI / num_phils)),
							(int) (center_x + (plate_location_radius - plate_radius)
									* java.lang.Math.cos((i + .5) * 2
											* java.lang.Math.PI / num_phils)),
					...
					win.drawLine(
							(int) (center_x + (plate_location_radius + plate_radius)
									* java.lang.Math.cos(+0.5
											* java.lang.Math
													.atan((float) plate_radius
															/ (float) plate_location_radius)
											+ (i) * 2 * java.lang.Math.PI
											/ num_phils)),
							(int) (center_y + (plate_location_radius + plate_radius)
